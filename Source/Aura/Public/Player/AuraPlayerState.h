...
public:
	AAuraPlayerState();
	virtual void GetLifetimeReplicatedProps(TArray<class FLifetimeProperty>& OutLifetimeProps) const override;//生命周期复制的道具
...
private:
	...
	UFUNCTION()
	void OnRep_AttributePoints(int32 OldAttributePoints);//网络同步 + 通知响应

	UFUNCTION()
	void OnRep_SpellPoints(int32 OldSpellPoints);//网络同步 + 通知响应
};
